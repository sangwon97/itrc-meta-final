...
registerOnce('tv-picker', {
  ...
  pick(clientX, clientY) {
    ...
    const hits = this.raycaster.intersectObjects(getTvMeshes(), true);
    const hit = hits.find((item) => item.object?.userData?.tvInfo);
    if (!hit) return;
    ...
    window.dispatchEvent(new CustomEvent('tv-select', { detail: hit.object.userData.tvInfo }));
  },
